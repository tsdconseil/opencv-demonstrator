...
# ifdef OCV240
  sortie.nb_sorties = 2;
# endif
  sortie.outname[1] = "ap/av";
  sortie.outname[1] = "Segmentation";
  osel = -1;
...
int SousArrierePlanDemo::calcul(Node &model, cv::Mat &I)
{
  ...
  algo->apply(tmp, mask);
  journal.trace("ok.");
  resize(mask,sortie.O[0],Size(0,0),4,4);
...
int OptFlowDemo::calcul(Node &model, cv::Mat &I)
{
  ...
  cvtColor(hsv, sortie.O[0], cv::COLOR_HSV2BGR);
  sortie.O[0].convertTo(sortie.O[0], CV_8UC3, 255);
  journal.verbose("fait: %d * %d, depth = %d.", sortie.O[0].cols, sortie.O[0].rows, sortie.O[0].depth());
  return 0;
}
