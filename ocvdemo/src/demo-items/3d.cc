...
# endif
  ...
  out.O[2] = disp8;
  out.nout = 3;
  out.outname[0] = "Image 1";
  out.outname[1] = "Image 2";
  out.outname[2] = langue.get_item("disp-map");
  return 0;
}
...
int CamCalDemo::calcul(Node &model, cv::Mat &I)
{
  ...
  out.O[0] = I;
  if(found)
   cv::drawChessboardCorners(out.O[0], board_size, Mat(pointbuf), found);
  ...
  if(found)
  {
    ...
    cv::undistort(Ior, out.O[1], cameraMatrix, distCoeffs);

    Size sz = I.size();
    sz.height = sz.width = max(sz.width, sz.height);
    sz.height = sz.width = max(sz.width, 500);

    out.O[2] = cv::Mat::zeros(sz, CV_8UC3);
