...
int InpaintDemo::calcul(Node &model, cv::Mat &I)
{
  if(params.masque.data != nullptr)
    cv::inpaint(I, params.masque, sortie.O[0], 3, CV_INPAINT_TELEA);
  else
    sortie.O[0] = I.clone();
  return 0;
}
...
int WShedDemo::calcul(Node &model, cv::Mat &I)
{
  ...
  sortie.O[0] = distTransformed.clone();
  ...
  sortie.O[2] = Mat(markers.size(), CV_8UC3);
  ...
  for(auto i = 0; i < markers.rows; i++)
  {
    for(auto j = 0; j < markers.cols; j++)
    {
      ...
      if(index == -1)
        sortie.O[2].at<Vec3b>(i,j) = Vec3b(255,255,255);
      else if((index <= 0) || (index > compCount))
        sortie.O[2].at<Vec3b>(i,j) = Vec3b(0,0,0);
      else
        sortie.O[2].at<Vec3b>(i,j) = colorTab[index - 1];
    }
  }
  sortie.O[2] = sortie.O[2] * 0.5 + I * 0.5;
  return 0;
}
...
int GrabCutDemo::calcul(Node &model, cv::Mat &I)
{
  ...
  for(auto y = 0; y < I.rows; y++)
  {
    for(auto x = 0; x < I.cols; x++)
    {
      ...
      {
        Vec3b &pix = sortie.O[0].at<Vec3b>(Point(x,y));
        pix[0] = 0;
        pix[1] = 0;
        pix[2] = 0;
      }
...
int Seuillage::calcul(Node &model, cv::Mat &I)
{
  ...
  if(sel == 0)
  {
    ...
    threshold(Ig, sortie.O[0], seuil, 255, THRESH_BINARY_INV);
  }
  // Otsu
  else if(sel == 1)
  {
...
int DTransDemo::calcul(Node &model, cv::Mat &I)
{
  ...
  journal.trace("ok.");
  normalize(tmp, sortie.O[0], 0, 255, NORM_MINMAX, CV_8UC1);
  return 0;
}
